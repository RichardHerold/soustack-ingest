  ...
  try {
    ...
    if (typeof converter === "function") {
      const result = await Promise.resolve(
        (converter as (input: string) => string | Promise<string>)(rtf),
      );
    ...
    if (converter && typeof (converter as { fromString?: unknown }).fromString === "function") {
      const result = await new Promise<string>((resolve, reject) => {
        (
          converter as {
            fromString: (input: string, cb: (err: Error | null, text?: string) => void) => void;
          }
        ).fromString(rtf, (error, text) => {
          if (error) {
            reject(error);
          } else {
            resolve(text ?? "");
...
export async function readRtf(filePath: string): Promise<AdapterOutput> {
  const text = await convertRtfToText(filePath);
