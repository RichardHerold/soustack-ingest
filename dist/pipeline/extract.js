...
function splitSections(lines) {
    ...
    const isInstructionHeader = (text) => /^(instructions?|directions|method|preparation|steps?|step\s*\d+)$/i.test(normalizeHeader(text));
    const cleanIngredient = (text) => text.replace(/^[-*]\s+/, "").trim();
    const cleanInstruction = (text) => text
        .replace(/^(step\s*\d+[:.)]?\s*)/i, "")
        .replace(/^\d+[.)]\s+/, "")
        .trim();
    const ingredientStarters = ["salt", "pepper", "pinch", "dash"];
    const isIngredientLike = (text) => /^[-*]\s+/.test(text) ||
